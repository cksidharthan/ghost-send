...
const getSecretByIDLocked = `-- name: GetSecretByIDLocked :one
SELECT
    s.id,
    CASE
        WHEN (s.password_hash = crypt($1, s.password_hash))
        THEN pgp_sym_decrypt(s.secret_text::bytea, $1)::text
        ELSE ''
    END as secret_text,
    s.password_hash,
    s.salt,
...
type GetSecretByIDLockedParams struct {
	Password string    `json:"password"`
	SecretID uuid.UUID `json:"secret_id"`
}

type GetSecretByIDLockedRow struct {
	ID              uuid.UUID    `json:"id"`
	SecretText      string       `json:"secret_text"`
	PasswordHash    string       `json:"password_hash"`
	Salt            string       `json:"salt"`
...
func (q *Queries) GetSecretByIDLocked(ctx context.Context, arg GetSecretByIDLockedParams) (GetSecretByIDLockedRow, error) {
	row := q.db.QueryRowContext(ctx, getSecretByIDLocked, arg.Password, arg.SecretID)
	var i GetSecretByIDLockedRow
	err := row.Scan(
		&i.ID,
		&i.SecretText,
		&i.PasswordHash,
