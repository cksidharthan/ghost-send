  ...
  <main class="h-full">
    ...
    <div class="relative min-h-[calc(100vh-theme(spacing.16)-theme(spacing.32))]">
      ...
      <div class="absolute inset-0">
        <!-- Grid pattern -->
        <div class="absolute inset-0 [background:radial-gradient(#1c274f_1px,transparent_1px)] [background-size:32px_32px] opacity-20"></div>
        ...
        <div class="absolute inset-0">
          <svg class="w-full h-full" viewBox="0 0 1200 800" fill="none" xmlns="http://www.w3.org/2000/svg">
            <!-- Diagonal lines -->
            <path 
              d="M-100 200L1300 -100" 
              stroke="url(#line-gradient)" 
              stroke-width="1" 
              stroke-opacity="0.1"
            />
            <path 
              d="M-100 400L1300 100" 
              stroke="url(#line-gradient)" 
              stroke-width="1" 
              stroke-opacity="0.1"
            />
            <path 
              d="M-100 600L1300 300" 
              stroke="url(#line-gradient)" 
              stroke-width="1" 
              stroke-opacity="0.1"
            />
            <path 
              d="M-100 800L1300 500" 
              stroke="url(#line-gradient)" 
              stroke-width="1" 
              stroke-opacity="0.1"
            />
            ...
            <!-- Curved accent lines -->
            <path
              d="M-100 700Q300 600 600 700T1300 700"
              stroke="url(#line-gradient)"
              stroke-width="1"
              stroke-opacity="0.2"
              class="animate-pulse"
            />
            <path
              d="M-100 750Q300 650 600 750T1300 750"
              stroke="url(#line-gradient)"
              stroke-width="1"
              stroke-opacity="0.1"
            />
        ...
        <!-- Subtle glow effects -->
        <div class="absolute top-0 -left-4 w-72 h-72 bg-indigo-500/10 rounded-full blur-3xl"></div>
        <div class="absolute bottom-0 -right-4 w-72 h-72 bg-purple-500/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <!-- Content Container -->
      <div class="mx-auto max-w-7xl px-6 py-12 h-full">
        <div class="flex flex-col lg:flex-row lg:items-start lg:gap-x-16 h-full">
          <!-- Left Side - Hero Content -->
          <div class="max-w-2xl lg:max-w-xl lg:sticky lg:top-24 mb-12 lg:mb-0">
            ...
            <p class="mt-6 text-lg sm:text-xl leading-8 text-gray-300">
          ...
          <div class="lg:flex-1 lg:min-w-[500px]">
            <div class="bg-gray-900/80 backdrop-blur-sm p-6 sm:p-8 rounded-xl shadow-2xl transition-all duration-300 hover:shadow-indigo-500/10 border border-gray-800/50">
              <div>
              ...
              <form @submit.prevent="submitSecret" class="mt-8 space-y-6">
                ...
                >
                  <span class="absolute left-0 inset-y-0 flex items-center pl-3">
                    <Icon name="heroicons:shield-check" class="w-5 h-5 text-indigo-500 group-hover:text-indigo-400 transition-colors duration-300" />
                  </span>
              ...
              <div v-if="result" class="mt-4 p-4 bg-gray-700/50 backdrop-blur-sm rounded-lg border border-gray-600">
                <div class="flex items-center justify-between">
                  ...
                  <button
                    ...
                    class="ml-4 p-2 text-gray-400 hover:text-white transition-colors duration-200"
                  >
...
const submitSecret = async () => {
  ...
  try {
    ...
    result.value = `Secret created successfully! Share this link: ${data.url}`
  } catch (error) {
